...
app.MapGet("/posts_offset", async (BloggingContext context,[FromQuery] int page) =>
{
    var offset = page * 10;
    var posts = await context.Posts
        .Where(p => !p.Deleted )
    ...
    var authors = await (from pa in context.PostAuthors
        ...
        select new { pa.PostId, Author = new { a.Id, a.Name, a.Slug, a.ThumbnailImage, a.FeaturedImage, a.CreatedAt, a.UpdatedAt } }).ToListAsync();
