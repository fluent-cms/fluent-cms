...
public class EntitiesController(IEntityService entityService) : ControllerBase
{
    ...
    {
        var items = await entityService.GetOne(entityName, id);
        return items is null ? NotFound() : Ok(items);
    }
    ...
    {
        var id = await entityService.Insert(entityName, new Record(item));
    ...
    {
        var id = await entityService.Delete(entityName, new Record(item));
        return id is null ? NotFound() : Ok(id);
    }
}